...
def run_self_diagnosis_experiment(wrapper: ModelWrapper, examples: List[ModelOutput], attribute_name: str, pattern: str,
    ...
    # Debugging code to count examples with None score for the specified attribute
    examples_with_none = [example for example in examples if example.scores.get(attribute_name) is None]
    print(f"Number of examples with None score for '{attribute_name}': {len(examples_with_none)}")
...
if __name__ == '__main__':
    ...
    # Print the scores of a few random examples
    print("Debugging: Printing scores of random examples")
    for i in range(5):
        random_example = random.choice(examples)
        print(f"Example {i+1} scores: {random_example.scores}")  
